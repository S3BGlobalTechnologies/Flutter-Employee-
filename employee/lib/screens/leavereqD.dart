...
class _HolidayRequestPageState extends State<HolidayRequestPage> {
  ...
  Future<void> _initializeDatabase() async {
    ...
    final path = '${directory.path}/holiday_requests.db';
    ...
    _database = await openDatabase(
      ...
      onCreate: (db, version) async {
        await db.execute(
          'CREATE TABLE holiday_requests(id INTEGER PRIMARY KEY, start_date TEXT, end_date TEXT, total_days INTEGER, reason TEXT, leave_type TEXT, username TEXT, user_id TEXT)',
        );
      },
    );
  }
  ...
  Future<void> _submitHolidayRequest() async {
    if (_formKey.currentState!.validate() &&
      ...
      try {
        ...
        await _database?.insert('holiday_requests', {
          'start_date': _startDate!.toIso8601String(),
          'end_date': _endDate!.toIso8601String(),
          'total_days': _totalDaysSelected,
          'reason': _reasonController.text,
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Holiday request submitted successfully!'), backgroundColor: Colors.green),
        );
        setState(() {
          _startDate = null;
          _endDate = null;
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error submitting request: ${e.message}'), backgroundColor: Colors.red),
        );
      }
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Please select both dates and fill the form'), backgroundColor: Colors.red),
      );
    }
  }
    ...
    return Scaffold(
      ...
      drawer: Drawer(
        child: ListView(
          ...
          children: [
            DrawerHeader(
              decoration: BoxDecoration(color: Color.fromARGB(255, 228, 125, 204)),
              child: Text(
                'Your Name',
